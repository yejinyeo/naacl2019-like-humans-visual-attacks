...
# Encoder 추출 후 GPU로 이동 및 추론 모드 설정
...
def preprocess_image(image_path):
    """
    이미지를 EasyOCR의 Encoder가 처리할 수 있도록 전처리.
    :param image_path: 이미지 파일 경로
    :return: 전처리된 이미지 Tensor
    """
    with Image.open(image_path) as img:
        img = img.convert("L")  # Grayscale 변환
        img = img.resize((32, 32))  # EasyOCR Encoder 입력 크기로 조정
        img = np.array(img) / 255.0  # 정규화 (0-1)
        img = torch.FloatTensor(img).unsqueeze(0).unsqueeze(0).to(device)  # (1, 1, 32, 32) 형태로 변경 후 GPU로 이동
    return img

def generate_vectors(image_dir, saving_chars_file, output_file):
    """
    이미지 -> EasyOCR Encoder 벡터 변환하여 vec.normalized 파일로 저장.
    :param image_dir: 이미지 디렉토리
    :param saving_chars_file: saving_chars.txt (문자 정보 포함)
    :param output_file: 벡터 저장 파일
    """
    # saving_chars.txt 읽기
    char_mapping = {}
    ...
    vector_dimension = 256  # EasyOCR encoder output size
    ...
    for image_name in image_files:
        ...
        try:
            # 이미지 전처리 후 encoder 통과
            img_tensor = preprocess_image(image_path)
            with torch.no_grad():
                feature_vector = encoder(img_tensor).mean(dim=(2, 3)).squeeze().cpu().numpy()  # 공간 차원 평균 후 Flatten
